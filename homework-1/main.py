...
with psycopg2.connect(host='localhost', database='north', user='postgres', password='95627102') as conn:
    with conn.cursor() as cur:
        with open('north_data/customers_data.csv', encoding='CP1251') as f:
        ...
        with open('north_data/employees_data.csv', encoding='CP1251') as f:
            reader = csv.DictReader(f)
            for row in reader:
        ...
        with open('north_data/orders_data.csv', encoding='CP1251') as f:
            reader = csv.DictReader(f)
            for row in reader:
